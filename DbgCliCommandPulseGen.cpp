...
void DbgCli_Command_PulseGen::execute(unsigned int argc, const char** args, unsigned int idxToFirstArgToHandle)
{
  ...
  TR_PRINT_STR(m_trPort, DbgTrace_Level::info, m_isRunning ? "Heart beat generator is running." : "Heart beat generator is inactive.")
}

bool DbgCli_Command_PulseGen::isRunning()
{
  return m_isRunning;
}
...
{
  if (m_newTimeMillis != m_currentTimeMillis)
  {
    startTimer();
  }
}
...
bool DbgCli_Command_PulseGen::isTimerRunning()
{
  bool isRunning = false;
  if (0 != m_randomIntervalTimer)
  {
    isRunning = m_randomIntervalTimer->isRunning();
  }
  return isRunning;
}
